...
program: PROGRAM LITERAL SEP_COLON varsDec* modulo* main;

main: FUNCTION 'main' LEFT_PAR RIGHT_PAR body;

modulo: tipo? FUNCTION LITERAL LEFT_PAR args RIGHT_PAR body;

tipo: ('int' | 'float' | 'string' | 'bool');

args: ((tipo LITERAL) (',' tipo LITERAL)*)?;
...
varsDec:
	tipo (
		LITERAL (
			(LEFT_SBRACKET (VAR_INT) RIGHT_SBRACKET) (
				LEFT_SBRACKET (VAR_INT) RIGHT_SBRACKET
			)?
		)? SEP_COMMA?
	)+;
...
exp: term ((PLUS | MINUS) exp)?;

term: factor ((TIMES | DIVISION) term)?;
